...
def main() -> None:
    ...
    for seed in [int(value) for value in args.seeds.split(",") if value.strip()]:
        ...
        seed_splits.append((seed, train_examples, test_examples))

    reports = estimate_repeated_split_embedding_workloads(
        seed_splits,
        cache_path=args.cache_path,
        k=args.semantic_depth,
        dataset=args.dataset,
    )
    output_csv = args.output_csv or Path("outputs/results") / f"{args.dataset}_repeated_embedding_preflight.csv"
    csv_path = write_embedding_workload_report(output_csv, *reports)
