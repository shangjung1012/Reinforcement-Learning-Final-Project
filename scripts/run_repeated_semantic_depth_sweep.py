...
from selective_rag_rl.experiments.retrieval_policy_experiment import AUTO_POLICY_MODEL_CHOICES, FEATURE_SET_CHOICES
from selective_rag_rl.experiments.semantic_depth_sweep import run_repeated_semantic_depth_sweep
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs") / "repeated_semantic_depth_runs")
    parser.add_argument("--seeds", default="41,42")
    parser.add_argument("--semantic-depths", default="5,8")
    parser.add_argument("--baseline-semantic-depth", type=int, default=None)
    ...
    parser.add_argument("--num-train-examples", type=int, default=20)
    ...
    semantic_depths = [int(value) for value in args.semantic_depths.split(",") if value.strip()]
    ...
    metadata = run_repeated_semantic_depth_sweep(
        ...
        baseline_semantic_depth=args.baseline_semantic_depth,
    ...
    print(pd.read_csv(metadata["depth_stability_csv"]).to_string(index=False))
